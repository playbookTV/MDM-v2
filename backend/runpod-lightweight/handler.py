"""
Lightweight RunPod Handler for Modomo AI Pipeline
Handles interior design scene analysis with runtime dependency loading
"""
...
def load_models():
    ...
    if not check_dependencies():
        logger.error("Dependencies not available, cannot load models")
        return False
...
def handler(event):
    """Main RunPod serverless handler with improved error handling"""
    try:
        ...
        if not models:
            ...
            if not load_models():
                return {
                    "status": "error",
                    "error": "Failed to load AI models - dependencies may still be installing"
                }
...
if __name__ == "__main__":
    ...
    if check_dependencies():
        ...
        logger.warning("⚠️ Some dependencies missing - will load at runtime")
